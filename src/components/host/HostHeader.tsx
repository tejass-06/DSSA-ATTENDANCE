...
import { Radio, ArrowLeft, History, LayoutDashboard } from "lucide-react";
...
interface HostHeaderProps {
  ...
  activeTab?: "live" | "history" | "detail";
}

export function HostHeader({ user, hasActiveSession = false, activeTab = "live" }: HostHeaderProps) {
  return (
    <div className="space-y-4 border-b border-white/[0.08] pb-6">
      ...
      <div className="flex items-center gap-2 pt-2">
        <Link
          href="/host"
          className={`inline-flex items-center gap-2 rounded-xl px-3.5 py-2 text-xs font-mono transition-colors ${
            activeTab === "live"
              ? "border border-cyan-500/40 bg-cyan-950/40 text-cyan-300 font-semibold"
              : "border border-white/10 bg-zinc-900/40 text-zinc-400 hover:text-white hover:bg-zinc-800/60"
          }`}
        >
          <LayoutDashboard className="h-3.5 w-3.5" />
          <span>Live Session Console</span>
        </Link>
        ...
        >
          ...
          <span>Hosted Sessions History</span>
        </Link>
      </div>
    </div>
  );
